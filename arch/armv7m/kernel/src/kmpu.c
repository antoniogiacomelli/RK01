...
/*
 * File intent:
 *   ARMv7-M MPU policy and validation. This code builds domain/shared-region
 ...
 *   - Context switch loads only prevalidated RBAR/RASR values from the TCB.
...
/*
 * MPU region size/alignment tests use the standard power-of-two identity:
 * exactly one bit set means value & (value - 1) is zero.
 */
...
static RK_ERR kMpuEncodeSize_(ULONG const regionSize,
                              ULONG *const sizeFieldPtr)
{
    ULONG value;
    ULONG log2Value;

    if (sizeFieldPtr == NULL)
    {
        return (RK_ERR_OBJ_NULL);
    }

    /* ARMv7-M encodes region size as log2(size) - 1 in RASR[5:1].
     * Hardware regions must be at least 32 bytes and power-of-two sized. */
    if ((regionSize < 32UL) ||
                       ...
                       ULONG const attributes)
{
    ...
    err = kMpuEncodeSize_(regionSize, &sizeField);
    ...
    /* Store precomputed RBAR/RASR values in the TCB. Context switch can then
     * reload regions without recalculating alignment, size, or attributes. */
    regionPtr->rbar =
        baseAddress | RK_MPU_RBAR_VALID |
        ((ULONG)regionNumber & RK_MPU_RBAR_REGION_MASK);

    regionPtr->rasr = attributes | sizeField | RK_MPU_RASR_ENABLE;
...
VOID kMpuInit(VOID)
{
    ...
    /* RK01 assumes the standard Cortex-M4 MPU with 8 data regions. */
    ...
    RK_REG_MPU_RASR = flashRegion.rasr;
